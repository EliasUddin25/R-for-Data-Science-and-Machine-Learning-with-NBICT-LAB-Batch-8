...
# Fit the model (formula of Two way ANOVA)
...
# Shapiro-Wilk on residulas (normality)
residuals(m)
res <- residuals(m)

shapiro.test(res)

# Levene's test for equal variances (car package needed)
library(car)
leveneTest(y ~ fert*irr, data = d, center = median)

# car:: leveneTest(y ~ fert*irr, data = d, center = median) (Alternative to previous two lines)

# Interaction contrasts (simple effects) : effect of fert  
# at each level of irr
library(emmeans)
contrast(emmeans(m, ~ fert | irr), method = "pairwise", adjust = "tukey")

# Interaction contrasts (simple effects) : effect of irr  
# at each level of fert
library(emmeans)
contrast(emmeans(m, ~ irr | fert), method = "pairwise", adjust = "tukey")

# Main‑effect comparisons (only if interaction is negligible)
# (m = name of two-way anova) (the following codes perform one-way anova)
